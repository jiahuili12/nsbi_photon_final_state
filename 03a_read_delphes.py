...
if args.delphes_run:
    ...
    os.makedirs("delpheslogs", exist_ok=True)
    delphes.run_delphes(
        delphes_directory=mg_dir + "/HEPTools/Delphes-3.5.0/",
...
def _dhh_to_point_symmetric(m_h1, m_h2, m1=120.0, m2=110.0):
...
def best_pairing_indices(j, dhh_tie_threshold=30.0):
    # Pairing logic:
    # 1) take leading 4 b-tagged jets
    # 2) test the 3 possible pairings
    # 3) define H1 as the higher-pT Higgs candidate
    # 4) choose the pairing with minimum symmetric d_hh to (120, 110)
    found, bjets = get_four_bjets(j)
    if not found:
...
def get_etaH_best(l, a, j, met, which=1):
    found, idx, _, _, _ = best_pairing_indices(j)
    if not found:
        return np.nan
    i1, i2, i3, i4 = idx
    ...
    if not found2:
        return np.nan
    if which == 1:
        return (bjets[i1] + bjets[i2]).eta
    return (bjets[i3] + bjets[i4]).eta

def get_deltaeta_hh_best(l, a, j, met):
    eta1 = get_etaH_best(l, a, j, met, which=1)
    eta2 = get_etaH_best(l, a, j, met, which=2)
    if not np.isfinite(eta1) or not np.isfinite(eta2):
        return np.nan
    return abs(eta1 - eta2)

def get_deltaeta_bb_best(l, a, j, met, which=1):
    found, idx, _, _, _ = best_pairing_indices(j)
    if not found:
        return np.nan
    i1, i2, i3, i4 = idx
    ...
    if not found2:
        return np.nan
    if which == 1:
        return abs(bjets[i1].eta - bjets[i2].eta)
    return abs(bjets[i3].eta - bjets[i4].eta)
...
def get_dphi_bb_best(l, a, j, met, which=1):
    found, idx, _, _, _ = best_pairing_indices(j)
    if not found:
        return np.nan
    i1, i2, i3, i4 = idx
    ...
    if not found2:
        return np.nan
    if which == 1:
        return abs(_delta_phi(bjets[i1].phi, bjets[i2].phi))
    return abs(_delta_phi(bjets[i3].phi, bjets[i4].phi))

def get_dphi_hh_best(l, a, j, met):
    found, idx, _, _, _ = best_pairing_indices(j)
    if not found:
        return np.nan
    i1, i2, i3, i4 = idx
    ...
    if not found2:
        return np.nan
    H1 = bjets[i1] + bjets[i2]
    H2 = bjets[i3] + bjets[i4]
    return abs(_delta_phi(H1.phi, H2.phi))
...
def add_observables(delphes):
    ...
    delphes.add_observable_from_function("deltaeta_hh", get_deltaeta_hh_best, required=False)
    delphes.add_observable_from_function("eta_H1", lambda l,a,j,met: get_etaH_best(l,a,j,met,which=1), required=False)
    delphes.add_observable_from_function("eta_H2", lambda l,a,j,met: get_etaH_best(l,a,j,met,which=2), required=False)
    delphes.add_observable_from_function("dphi_hh", get_dphi_hh_best, required=False)

    # Pair-level angular observables
    delphes.add_observable_from_function("deltaeta_bb1", lambda l,a,j,met: get_deltaeta_bb_best(l,a,j,met,which=1), required=False)
    delphes.add_observable_from_function("deltaeta_bb2", lambda l,a,j,met: get_deltaeta_bb_best(l,a,j,met,which=2), required=False)
    delphes.add_observable_from_function("dphi_bb1", lambda l,a,j,met: get_dphi_bb_best(l,a,j,met,which=1), required=False)
    delphes.add_observable_from_function("dphi_bb2", lambda l,a,j,met: get_dphi_bb_best(l,a,j,met,which=2), required=False)
