...
def best_pairing_indices(j, dhh_tie_threshold=30.0): #pairing 这里注意一下，可能bjet多于4对，要用for loop了，现在
    found, bjets = get_four_bjets(j)                 #“假设最有用的 Higgs candidate 总是来自 leading 4 b-jets”这对signal没什么问题，但是background来说，他本来就是没有真正的Higgs的，4个bjet里哪个组合成Higgs candidate都不太合理，所以可能需要改成对所有bjet组合进行循环，找到最好的那个组合（也就是dhh最小的那个组合）。不过先按照现在这个假设来，如果以后需要改的话再说。
