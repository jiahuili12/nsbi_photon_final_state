...
def get_all_bjets(j):
    # Return all b-tagged jets, sorted by pT
...
def get_four_bjets(j):
    ...
    bjets = sorted([jet for jet in j if jet.b_tag == 1], key=lambda x: x.pt, reverse=True)  # b jet meaning
    if len(bjets) >= 4:
        return True, bjets[:4]
    return False, None
...
def best_pairing_indices(j, dhh_tie_threshold=30.0):
    # OLD VERSION:
    # found, bjets = get_four_bjets(j)
    # “假设最有用的 Higgs candidate 总是来自 leading 4 b-jets”
    #
    # NEW VERSION:
    # If there are more than 4 b-tagged jets, loop over all 4-jet combinations
    # from all b-tagged jets, and for each 4-jet set test the 3 possible pairings.
    #
    # Return:
    # (found,
    #  (iH1a, iH1b, iH2a, iH2b),     # indices in the ALL-bjet list
    #  (pTH1, pTH2),
    #  (mH1, mH2),
    #  dhh_best)

    found, bjets = get_all_bjets(j)
    if not found:
        return False, None, None, None, None

    results = []

    # Loop over all ways to choose 4 jets from all tagged b-jets
    for quad in itertools.combinations(range(len(bjets)), 4):
        q0, q1, q2, q3 = quad
